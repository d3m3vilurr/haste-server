...
FileDocumentStore.prototype._toFilename = function(key) {
  return this.basePath + '/' + FileDocumentStore.md5(key);
};
...
FileDocumentStore.prototype.append = function(key, data, callback, skipExpire) {
  var _this = this;
  var fn = this._toFilename(key);
  fs.exists(fn, function(exist) {
    if (!exist) {
      return _this.set(key, data, callback, skipExpire);
    }
    try {
      fs.appendFile(fn, data, 'utf8', function(err) {
        if (err) {
          callback(false);
        }
        else {
...
FileDocumentStore.prototype.clear = function(key, callback) {
  var fn = this._toFilename(key);
  fs.exists(fn, function(exist) {
    if (!exist) {
